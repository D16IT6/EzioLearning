	...
	UserManager<AppUser> userManager, IStringLocalizer<CourseController> localizer) : ControllerBase
{
	...
	[HttpGet]
	public async Task<IActionResult> GetCourse(int pageNumber, int pageSize)
	{
		var pagedResult = await courseRepository.GetPageWithDto<CourseInGridViewDto>();

		return Ok(new ResponseBaseWithData<PageResult<CourseInGridViewDto>>()
		{
			Data = pagedResult,
